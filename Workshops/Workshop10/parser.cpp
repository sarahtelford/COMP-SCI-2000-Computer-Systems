...
static ast parse_declaration()
{
    ...
    mustbe(tk_semi) ;
    var = declare_variable(id,type,"local") ;
...
static ast parse_let()
{
    ...
    id = lookup_variable(identifier) ;
    //alt to the two lines above:
...
static ast parse_expression()
{
    ...
    if( have(tg_infix_op) ){     // if the expression has an infix operator - execute this block
